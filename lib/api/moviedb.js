...
module.exports = ({ apiKey }) => {
    ...
    return request(queryUrl)
      .then(response => {
        if (response.statusCode === 429) {
          return Promise.delay(10000).then(() => getMovieDetails(id));
        } else {
  ...
  getTVDetails = (id) => {
    ...
    return request(queryUrl)
      .then(response => {
        if (response.statusCode === 429) {
          return Promise.delay(10000).then(() => getTVDetails(id));
        } else {
          return JSON.parse(response.body);
        }
      });
  ...
  getEpisodeInfo = (id, episodeInfo) => {
    ...
    return request(queryUrl).then(function(response) {
      if (response.statusCode === 429) {
        return Promise.delay(10000).then(() => getEpisodeInfo(id, episodeInfo));
      } else {
        return JSON.parse(response.body);
      }
    });
