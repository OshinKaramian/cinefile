 ...
 */
const modifyFilenameForNextSearch = function(filename, category, direction) {
  ...
  if (category === 'tv') {
    const seasonRegex = 
      filename.match(/(?:S)([0-9][0-9])(?:E)([0-9][0-9])/i) ||
      filename.match(/[0-9]?[0-9](?:[-x]?)[0-9][0-9]/i);
    
    if (seasonRegex) {
      filename = filename.slice(0, seasonRegex.index + seasonRegex[0].length);
    }
  }
  ...
  if (/[1-2][0-9][0-9][0-9]/.test(filename)) {
    const yearMatch = filename.match(/[1-2][0-9][0-9][0-9]/);
    filename = filename.slice(0, yearMatch.index - 1);
    lastIndex = yearMatch[0];
  } else {
    filename = filename.split('+');
    let lastIndex = direction === 'forwards' ? filename.shift() : filename.pop();
    filename = filename.join('+');
  }
  ...
  const correctMatch = results
      ...
      .map(response => {
        const massageForMatch = (massage) => 
          massage.replace('\'', '').replace(/[+]|[^\w\s]|\s+/gi, ' ').toLowerCase().trim();
...
module.exports = ({ moviedbKey }) => {
  ...
  const queryMovieDbForMatch = async ({ filename, mediaType, year, searchTerm, direction = 'backward' }) => {
    ...
    if (statusCode === 200) {
      ...
      if (total_results === 0) {
        const { year, filename: newSearchTerm } = modifyFilenameForNextSearch(searchTerm, mediaType, direction); 
  ...
  return async ({ filename, mediaType, year }) => {
    ...
    if (queryInfoBackwards.match_year) {
      ...
      if (queryInfoBackwards.match_percentage === queryInfoForwards.match_percentage) {
        if (queryInfoBackwards.name.length === queryInfoForwards.name.length) {
          queryInfo = queryInfoBackwards.name.split(' ') > queryInfoForwards.name.split(' ') ? queryInfoBackwards : queryInfoForwards;
        } else {
